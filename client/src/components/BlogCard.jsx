...
const BlogCard = () => {
  ...
  <div className="grid xl:grid-cols-3 md:grid-cols-2 grid-cols-1 gap-10 xl:gap-12">
    {blogs.map((item, index) => (
      <div
        className="md:w-[100%] md:h-[500px] flex flex-col cursor-pointer"
        key={index}
  ...
  {selectedCard && (
    <div className="fixed inset-0 bg-black/70 flex justify-center items-center z-50 !p-4">
      <div className="bg-gray-950 dark:bg-neutral-900 rounded-2xl max-w-2xl w-full !p-6 relative">
        ...
        <button
          onClick={handleClose}
          className="absolute !top-7 !right-7 !text-white hover:!text-black text-sm bg-black hover:bg-white !px-2 !py-1 cursor-pointer rounded-full"
        >
          ✕
        </button>
