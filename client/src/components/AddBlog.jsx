...
const AddBlog = () => {
  ...
  const handleDeleteBlog = async (Id) => {
    try {
      const response = await axios.delete(`http://localhost:5000/api/blog/deleteBlog/${Id}`, { withCredentials: true })
      fetchBlogs()
    } catch (error) {
      console.log(error)
      alert(error)
  ...
  const handleEditBlog = async (e) => {
    try {
      ...
      const response = await axios.put(`http://localhost:5000/api/blog/editBlog/${blogsId}`, formdata, {
        headers: {
          "Content-Type": "multipart/form-data"
        },
        withCredentials: true
    ...
    <div>
        ...
        blogs.map((item, index) => (
          <div className="!p-3 " key={index}>
            <div className="bg-gray-900  text-white rounded-2xl overflow-hidden shadow-md max-w-sm">
              ...
              {/* Content */}
              <div className="!p-4">
                <p className="text-sm text-gray-400">{new Date(item.date).toLocaleDateString("en-GB")}</p>
      ...
      {/* ======================================= edit blog =================================  */}
      {
        editBlog && (
          <div className="bg-black/80 fixed inset-0 flex items-center justify-center">
            <div className="flex flex-col min-w-5xl bg-gray-800 rounded-2xl !p-5">
              <h1 className="font-semibold text-2xl text-center !mb-8">edit a Blog</h1>
              <label className="!mb-1">Image</label>
              <input
                type="file"
                onChange={(e) => setImage(e.target.files[0])}
